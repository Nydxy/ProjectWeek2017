...
enum class QuestionType { Completion, Choice, TrueFalse }; //题型

//class of a question for user
class Question
{
private:
...
public:
	...
	Question() {}
	Question(QuestionOption questionOption, QuestionType questionType = QuestionType::Completion); //构造函数
...
string Question::FloatToString(float f)//float转string(最多保留1位小数)
{
	char str[200];
...
string Question::CreateChoice(int & rightchoice) //生成选择题选项
{
...
string Question::CreateTrueFalse(bool & judge)  //生成判断题和答案
{
...
void Question::GenerateExpression()//生成表达式
{
	auto GetOperator = []
	{
		//在这里可以设置各符号出现的概率
...
Question::Question(QuestionOption questionOption, QuestionType questionType)
{
